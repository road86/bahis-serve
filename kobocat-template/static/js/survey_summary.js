...
function get_form_shortname(id_string) {
    var d = ''
    if(id_string) {
        $.ajax({
            url: '/study/get_form_shortname/',
            type: 'POST',
            async: false,
            data: {'id_string': id_string},
            success: function (data) {
...
function get_form_permission(id_string){
    var perm = ''
    if(id_string) {
        $.ajax({
            url: '/study/get_form_permission/',
            type: 'POST',
            async: false,
            data: {'id_string': id_string},
            success: function (data) {
...
function createTableRow(dataObj,submission,jsondata,title, ownership,form_permission){
    ...
    //var study = get_study(dataObj)
    //var form_shortname = get_form_shortname(dataObj)
    //var form_perm = get_form_permission(dataObj)
...
console.log(isOwner+'--'+dataObj+'Can_View::'+Can_View+'Can_submit::'+Can_submit+'Can_edit::'+Can_edit);
    ...
    if ( isOwner ) {
	    ...
	    edit = '<a href="/'+jsonOwner[dataObj][1]+'/form_replace/'+dataObj+'/" class="btn red btn-md" type="button"> Update </a>';
        schedule = '<a href="/bhmodule/'+jsonOwner[dataObj][1]+'/forms/'+dataObj+'/'+'form_settings'+'" class="btn red btn-md" type="button"> Configuration </a>';
    ...
    if(Can_submit){
       newSubmission = '<a href="/add/'+dataObj+'/" target="_blank" class="btn red btn-md" type="button">New Submission</a>';
    }

    operation_status = '<a class="proj_submission" href="/approve-status/operational-status/"> Op-Status </a>';
    var table = $('#tg-xY4Sf');
    var spDataTableRow = $('<tr></tr>');
    //var spTableRowData = $('<td class="tg-yw4l">'+title + '</td><td>'+newSubmission+' '+details+' '+ projectSettings +' '   + roleSettings +' ' + edit+' ' + schedule + '</td>');
    var spTableRowData = $('<td class="tg-yw4l">'+title + '</td><td>'+details+' ' + edit+' ' + schedule + '</td>');
