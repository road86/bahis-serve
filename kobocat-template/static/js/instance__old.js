...
function createTable(canEdit,datavals) {
    var form_uid = datavals['formhub_uuid'];
    var form_data_uid = datavals['meta_instanceID'].replace('uuid:','');
    var keyValArr = {};
    ...
    if (languages.length > 1) {
        var languageRow = $('<div></div>');
        var languageStr = $('<label class="control-label">' + gettext("Change Language:") + '</label><br>');
        var languageSelect = $('<select class=" form-control language" style="margin: 10px 0 10px 0"></select>');
        var i;
        for (i = 0; i < languages.length; i++) {
            var language = languages[i];
            var languageOption = $('<option value="' + i + '">' + language + '</opton>');
    ...
    for (key in questions) {
        ...
        if(question.type != 'note'){
            ...
            var childQuestions = {};
                ...
                //console.log('Image block started*******************************************************');
                
                //console.log(key);
                src_url = form_uid+'/'+form_data_uid+'/'+datavals[key]
                //var src = _attachment_url(src_url, 'small');
                var src = 'media/'+form_owner+'/attachments/'+src_url
                //console.log('Image block running*******************************************************');
                //console.log(src)
                //var href = _attachment_url(src_url, 'medium');
                var href = 'media/'+form_owner+'/attachments/'+src_url
                //console.log(href)
